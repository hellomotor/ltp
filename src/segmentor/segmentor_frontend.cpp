...
namespace ltp {
    namespace segmentor {
        ...
        void SegmentorFrontend::process_instance(const std::vector<std::string> &lines,
                                                 uint32_t from,
                                                 uint32_t to,
            ...
            for (uint32_t i = from; i < to; ++i) {
                ...
                if (line.length() == 0) {
                    WARNING_LOG("read inst: line #%d, is empty", size_t(nr_lines));
                    continue;
                }
                            ...
                            for (size_t j = 0; j < num_chars; ++j) {
                                if (1 == num_chars) { output.con.append(1 << __s_id__); }
                                    ...
                                    if (0 == j) {
                                        ...
                                        output.con.append(
                                                (1 << __b_id__) | (1 << __i_id__) | (1 << __e_id__) | (1 << __s_id__));
                                    }
                                }
                            }
                        }
        ...
        bool SegmentorFrontend::read_instance(const char *train_file) {
            ...
            std::string line;
            boost::atomic_int nr_lines(0),
                        nr_fully_segmented(0),
                    nr_partially_segmented(0);
            ...
            while (std::getline(ifs, line, '\n')) {
                ...
                INFO_LOG("reading reference dataset at line #%d", ++lineno);
            }
            boost::thread_group threads;
            // unsigned int nthreads = std::thread::hardware_concurrency();
            unsigned int nthreads = 1;
            size_t npces = lines.size() / nthreads;
            for (uint i = 0; i < nthreads; ++i) {
                ...
                threads.create_thread(
                        boost::bind(&SegmentorFrontend::process_instance,
                                    this,
                                    boost::ref(lines),
                                    from,
                                    to,
                                    boost::ref(preprocessor),
                                    boost::ref(nr_lines),
                                    boost::ref(nr_fully_segmented),
                                    boost::ref(nr_partially_segmented)
                        ));
            }
            threads.join_all();

            INFO_LOG("report: input data size: ", train_data_input.size());
            INFO_LOG("report: output data size: ", train_data_output.size());

            INFO_LOG("report: loaded %d fully segmented data.", size_t(nr_fully_segmented));
            INFO_LOG("report: loaded %d partially segmented data.", size_t(nr_partially_segmented));
            return true;
        }
            ...
            for (size_t iter = 0; iter < train_opt.max_iter; ++iter) {
                ...
                if (f > best_f) {
                    ...
                    new_model->save(model_header.c_str(),
                                    (train_opt.dump_model_details ? Parameters::kDumpDetails
                                                                  : Parameters::kDumpAveraged),
                                    ofs);
